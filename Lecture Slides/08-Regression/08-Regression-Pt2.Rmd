...
---
# feols and stepwise estimation

With `feols()` we can combine .hi-blue[stepwise functions]on the RHS with .hi-red[multiple dependent variables] on the LHS, all within the same object!

Consider the (not valid) regression of 
* Claims, housing values and income on population
* Stepwise through 5 different FE specifications

```{r}
regs_both <- feols(c(claims, housing_value, income) ~ population | #<<
    sw0(state_fips, county_fips, state_fips + year, county_fips + year), 
    data = wl_df, vcov = "hetero") 
```
...
Which yields a `fixest_multi` object with .hi-slate[15 different model estimates!]

We can access its elements with five keys:
...
We can perform .hi-blue[basic access] of elements using the `object[key = value]` syntax 

For example, we can access the first LHS with
...
In this case, all our models are estimated on the same sample, and R tells us if we act a fool
...
---
# etable and fixest_multi

We can combine this syntax with `etable()` to plot specific estimates at once:

```{r error=TRUE}
etable(regs_both[fixef = 1])
```

---
# etable and fixest_multi

The order in which we put our keys will .hi-medgrn[re-order the columns]
* default is `sample > lhs > fixef > rhs > iv`
* set key to logical to include/omit all

```{r error=TRUE}
etable(regs_both[lhs = 2, fixef = TRUE])
```
---
# etable and fixest_multi
...
```{r error=TRUE}
etable(regs_both[fixef = 2:4, lhs = 2])
```
...
knitr::include_graphics("images/sah_table2.png")
```
---
