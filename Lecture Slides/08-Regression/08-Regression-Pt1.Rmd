...
knitr::include_graphics("images/wetlands.png")
```
...
`tidy()` and `confint()` make it easy to .hi-medgrn[plot regression output] rather than report in a table.


```{r, eval = F}
confint_1b <- confint(floodreg_1b, parm = c("wetland_ha"), level = 0.95)
data.frame(var = "wetland_ha", 
           estimate = tidy(floodreg_1b)[2, "estimate"],
ci_l = confint_1b[1,1],
ci_u = confint_1b[1,2]
) %>%
  ggplot(aes(x = var)) +
  ...
  lims(y = c(0,10)) +
  labs(y = "Coefficient (Claims $/Ha)", x = NULL) +
  theme_minimal()
```
