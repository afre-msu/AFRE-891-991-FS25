...
```{r, echo = F, out.width = "42%"}
knitr::include_graphics("images/DEM_Everest.jpg")
```
...
knitr::include_graphics("images/raster_ex.png")
```
...
.pull-right[
```{r, echo = F, out.width = "100%"}
knitr::include_graphics("images/PRISM_ppt_early_4kmD2_MTD_20230406.png")
```
]
...
.pull-right[
```{r, echo = F, out.width = "100%"}
knitr::include_graphics("images/landsat9_bands.png")
```
...
knitr::include_graphics("images/BlackMarble.jpg")
```
...
.pull-right[
```{r, echo = F, out.width = "100%"}
knitr::include_graphics("images/nlcd.jpg")
```
]
...
```{r, echo = F, out.width = "100%"}
knitr::include_graphics("images/cdl.png")
```
...
.pull-right[
```{r, echo = F, out.width = "100%"}
knitr::include_graphics("images/HI_landcover_analysis_NOAA.jpg")
```
]
...
ggplot()+
  geom_sf(data = counties, aes(fill = SQMILES))
```
]
...
```{r, out.height = "70%"}
tract_buff <- st_buffer(tracts, dist = 40233.6)

ggplot() +
  geom_sf(data = tract_buff[1,], fill = "navy", alpha = 0.4) +
  geom_sf(data = tracts[1,], fill = "yellow", alpha = 0.7) +
  maptheme
# crop to Wayne County with 25mi buffer:
tmax_crop <- terra::crop(tmax, tract_buff)
...
extr_sum <- group_by(extr_raster, ID) %>%
  summarise(mean_tmax = mean(PRISM_tmax_stable_4kmD2_20230404_bil))
