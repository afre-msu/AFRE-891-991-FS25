...
*While* we're on the topic... let's consider a .hi-purple[while loop]:
  * Continue to do an action *while* a condition is met
  * Make sure to increment i else the loop will run forever!
  
```{r}
i <- 1
while  (i < 8){
  print(paste(i, "is less than 8"))
  i <- i + 1
}
```
...
We can nest `if` into a `while` to .hi-red[stop the loop even if the while condition is TRUE]

```{r}
num <- 1

while (num < 8){
  print(paste(num/2, "is half of", num))
  num <- num + 1
...
```{r}
for (state in c("Indiana", "Michigan", "Colorado", "michigan")){
  if (state == "Michigan"){
...
If we  .hi-red[don't specify return values], we get back the output from the .hi-red[last computed step]
...
Note that if instead you save the final step to memory, you instead .hi-red[get nothing]
...
We can .hi-blue[specify return values] with `return()`
  ...
  * Necessary when you save the last step to memory
  
```{r}
square_plus3 <- function(x){ 
  x_sq <- x^2 # assign squared value as intermediate object
  x_sq_pl_3 <- x_sq + 3 
  text <- paste("The Square of", x, "+3 is", x_sq_pl_3)
 return(x_sq_pl_3) #<<
}
square_plus3(2)
```
...
murders_exp <- square_def(population, murders)
murders_exp <- square_def(total, murders)
```
]
...
murders_exp <- square_inj_1(murders, total, "square_of_total") %>%
  square_inj_1(rate, "rate_squared") %>%
  square_inj_1(population, "pop_square")
```
...
```{r}
square_inj_1(murders, rate, "rate_sq") %>% select(rate, rate_sq) %>% head()
```
---
...
We can also use an anonymous (lambda) function in a `map()` too!
...
Add the `.progress = TRUE` argument to any `map()` function to get a visible progress bar
```{r, eval = F}
map_many <- map(
  seq(1:2000000), 
  .f = ~ sqrt(.x) + 3, 
...
Like with `map()` functions, `future_map()` you can also add a progress bar with `.progress = T`
...
# Running over the same long vector in parallel:
tic()
square_parallel_long <- future_map(1:1000, slow_square, .progress = T)
toc()
...
To .hi-purple[customize the progress-tracking] process,  we can use the  .hi-slate[progressr] package.<sup>5</sup>
...
.footnote[<sup>5</sup> Checking the documentation of `future_map()` notes that the `.progress` argument __will__ be deprecated in the future, and recommends  .hi-slate[progressr] methods.]
---
# Progress with *progressr*
